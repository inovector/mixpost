...
const { activeTab, tabs, isDownloading, isDeleting, downloadExternal, deletePermanently } =
  useMedia()
...
<template>
    ...
    <div class="w-full row-px">
      <Tabs>
        <template v-for="(tabName, tabKey) in tabs" :key="tabKey">
          <Tab :active="activeTab === tabKey" @click="activeTab = tabKey">{{ tabName }}</Tab>
        </template>
      </Tabs>
    </div>
  ...
  >
    <template #header> Delete media </template>
    <template #body> Are you sure you want to delete selected media items? </template>
    <template #footer>
