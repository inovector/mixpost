...
<template>
  ...
  >
    ...
    <template #body>
      ...
      <Tabs>
        <template v-for="(tabName, tabId) in tabs" :key="tabId">
          <Tab :active="activeTab === tabId" @click="activeTab = tabId">{{ tabName }}</Tab>
        </template>
      </Tabs>
    ...
    <template #footer>
      ...
      <template v-if="selectedItems.length">
        <SecondaryButton v-tooltip.top="'Dismiss'" class="mr-xs" @click="deselectAll">
          <XIcon class="w-5! h-5!" />
        </SecondaryButton>

        <PrimaryButton @click="insert">Insert {{ selectedItems.length }} items </PrimaryButton>
      </template>
    </template>
  </DialogModal>
</template>
