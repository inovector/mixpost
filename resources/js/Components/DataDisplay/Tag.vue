...
const renameTag = () => {
  ...
  router.put(
    route('mixpost.tags.update', { tag: props.item.uuid }),
    {
      action: 'name',
  ...
  <div>
    <div
      :style="{
        ...
        color: lightOrDark(item.hex_color) === 'light' ? colorLight : colorDark
      }"
      :class="badgeClass"
    ...
    <template v-if="editable">
      <DialogModal :show="changeColorModal" max-width="md" @close="closeChangeColorModal">
        <template #header> Change label color </template>
        <template #body>
          <template v-if="changeColorModal">
            <div
              :style="{
                backgroundColor: changeColorHex,
                color: lightOrDark(changeColorHex) === 'light' ? colorLight : colorDark
              }"
              :class="badgeClass"
              class="w-fit"
            >
