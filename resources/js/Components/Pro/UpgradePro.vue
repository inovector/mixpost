...
<template>
  <template v-if="!$slots.trigger">
    <div role="button" class="relative" @click="open">
      <ProLabel />
    </div>
  </template>
  ...
  <DialogModal :show="show" max-width="lg" :closeable="true" :scrollable-body="true" @close="close">
    <template #header>
      <Flex>
        <LockOpen />
        <span
          class="bg-clip-text text-transparent font-medium bg-linear-to-r from-purple-500 to-indigo-500"
    ...
    <template #body>
      ...
      <p class="mt-xs">
        Mixpost Pro gives powerful tools for efficient social media management, saving time and
        boosting collaboration for impactful content creation.
      </p>
            ...
            <li class="flex items-center gap-xs mb-xs last:mb-0">
              ...
              <Flex>
                ...
                <template v-if="feature.desc">
                  <span
                    ><PureButton class="inline-block">
                      <template #icon>
                        <QuestionMarkCircle
                          v-tooltip="feature.desc"
                          class="w-5! h-5!"
                        /> </template></PureButton
                  ></span>
                </template>
