...
<template>
    <Alert v-if="reachedMaxCharacterLimit" variant="error" :closeable="false" class="mb-2">
        [{{ name }}] : You have reached the maximum character limit
    </Alert>

    <Alert v-if="mainContent.media.length > 4" variant="warning" class="mb-2">[{{ name }}]: Twitter accepts a maximum of 4 pictures. Don't worry, if you have selected several pictures, Mixpost will only post the first 4.</Alert>
