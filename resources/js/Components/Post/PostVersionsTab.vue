...
<template>
  <div>
      ...
      <Tabs class="mr-xs">
        <template v-for="(version, index) in versionsWithAccountData" :key="version.account_id">
          ...
          >
            ...
            />
            <span
              v-if="version.is_original && nameOfLastAvailableAccount"
            ...
            <VerticallyScrollableContent max-height="xl">
              <template v-for="account in availableAccounts" :key="account.id">
                <DropdownItem as="button" @click="$emit('add', account.id)">
                  <span class="mr-xs">
                    <Account :provider="account.provider" :img-url="account.image" :active="true" />
                  </span>
    ...
    >
      <template #header> Remove version </template>
      <template #body>
        <span v-if="confirmationRemoval">
          Are you sure you would like to delete version for
          <span class="capitalize">[{{ confirmationRemoval.account.provider }}]</span>
          {{ confirmationRemoval.account.name }}?
        </span>
      </template>
      <template #footer>
        <SecondaryButton class="mr-xs" @click="closeConfirmationRemoval">Cancel</SecondaryButton>
