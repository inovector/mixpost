...
const schedule = (postNow = false) => {
  ...
  axios
    .post(route('mixpost.posts.schedule', { post: postId.value }), {
      postNow
    })
    .then(response => {
...
<template>
  <div class="w-full flex items-center justify-end bg-stone-500 border-t border-gray-200 z-10">
    <div class="py-4 flex items-center space-x-xs row-px">
      ...
      <div class="flex items-center" role="group">
        <SecondaryButton
          size="md"
          :class="{
            'normal-case! border-r-indigo-800 rounded-r-none': scheduleTime,
            'rounded-r-lg!': !canSchedule
          }"
    ...
    <ConfirmationModal :show="confirmationPostNow" @close="confirmationPostNow = false">
      <template #header> Confirm publication </template>
      <template #body>
      ...
      <template #footer>
        <SecondaryButton class="mr-xs" @click="confirmationPostNow = false">Cancel</SecondaryButton>
