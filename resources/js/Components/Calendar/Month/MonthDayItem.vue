...
<template>
  ...
  >
    <div class="absolute w-full top-0 left-0 mt-xs text-center">
      <span
        class="w-7 h-7 inline-flex items-center justify-center p-1 mr-xs rounded-full text-gray-700"
        :class="{ 'bg-indigo-500 text-white': isToday, 'text-gray-400': day.isDisabled }"
        >{{ label }}</span
      >
    </div>
