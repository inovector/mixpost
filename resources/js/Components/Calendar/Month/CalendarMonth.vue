...
const previousMonthDays = computed(() => {
  ...
  const previousMonthLastMondayDayOfMonth = getDate(
    subDays(
      new Date(`${currentMonthDays.value[0].date}T00:00:00`),
  ...
  return [...Array(visibleNumberOfDaysFromPreviousMonth)].map((day, index) => {
    const date = new Date(
      `${getYear(previousMonth)}-${(getMonth(previousMonth) + 1).toString().padStart(2, '0')}-${(previousMonthLastMondayDayOfMonth + index).toString().padStart(2, '0')}T00:00:00`
    )
...
const currentMonthDays = computed(() => {
  return [...Array(numberOfDaysInMonth.value)].map((day, index) => {
    const date = new Date(
      `${year.value}-${month.value}-${(index + 1).toString().padStart(2, '0')}T00:00:00`
    )
...
const nextMonthDays = computed(() => {
  ...
  return [...Array(visibleNumberOfDaysFromNextMonth)].map((day, index) => {
    const date = new Date(
      `${getYear(nextMonth)}-${(getMonth(nextMonth) + 1).toString().padStart(2, '0')}-${(index + 1).toString().padStart(2, '0')}T00:00:00`
    )
...
<template>
  <div class="bg-white">
    ...
    <Weekdays :week-starts-on="weekStartsOn" />
