...
<template>
  <div class="w-full h-full flex flex-col py-2xl bg-white border-r border-gray-200">
    ...
    <div class="flex flex-col space-y-lg overflow-y-auto px-xl mt-2xl h-full">
      ...
      <MenuGroupBody>
        <MenuItem :url="route('mixpost.posts.index')" :active="$page.component === 'Posts/Index'">
          <template #icon>
            <GridIcon />
          </template>
          Posts
      ...
      <MenuDelimiter />
      <MenuGroupHeader :create-url="route('mixpost.posts.create')"> Configuration </MenuGroupHeader>
      <MenuGroupBody>
        <MenuItem
          :url="route('mixpost.accounts.index')"
          :active="$page.component === 'Accounts/Accounts'"
        >
          <template #icon>
            <ShareIcon />
          </template>
      ...
      <MenuDelimiter />
      <MenuGroupHeader> System </MenuGroupHeader>
      <MenuGroupBody>
        <MenuItem
          :url="route('mixpost.system.status')"
          :active="$page.component === 'System/Status'"
        >
          <template #icon>
            <InformationCircle />
          </template>
        ...
        <MenuItem
          ...
          :active="$page.component === 'System/Logs'"
        >
          <template #icon>
            <Document />
          </template>
        ...
        <template v-if="$page.props.app.horizon_path">
          <MenuItem
            :url="`/${$page.props.app.horizon_path}`"
            :external="true"
    ...
    <div class="absolute bottom-0 mb-sm w-full">
      ...
      <div class="flex flex-col items-start px-xl mt-sm">
        <div class="text-sm text-gray-500 mb-xs">
          Lite version: {{ $page.props.mixpost.version }}
        </div>
        <UpgradePro>
          <template #trigger>
